	...
	<div class="box box-widget">
		...
		<?php echo form_open_multipart($this->uri->uri_string(), 'class="form-horizontal"'); ?>
			...
			<div class="box-body">
				...
				<?php echo form_label(lang('sports_current_icon') , 'icon', array('class' => 'col-md-2 control-label') ); ?>
					<div class="col-sm-4">
						 <img src="<?php echo base_url();?>uploads/sports/<?php echo set_value('current_icon', isset($sports->icon) ? $sports->icon : ''); ?>" style="height:30px;"/>
						<input id="current_icon" type="hidden" name="current_icon" value="<?php echo set_value('current_icon', isset($sports->icon) ? $sports->icon : ''); ?>"/>				  
					</div>
				</div>
				...
				<div class="box-footer">
					<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('sports_action_edit'); ?>" />
					<?php echo anchor(SITE_AREA . '/dashboard/sports', lang('sports_cancel'), 'class="btn btn-warning"'); ?>
			   
					<?php if ($this->auth->has_permission('Sports.Content.Delete')) : ?>
					<button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('sports_delete_confirm'))); ?>');">
						<span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('sports_delete_record'); ?>
					</button>
					<?php endif; ?>
				</div>
			</div>
