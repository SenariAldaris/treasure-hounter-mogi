...
if (isset($_POST['cnt']) && isset($_POST['num']) && isset($_SESSION['view']) && $_POST['cnt'] == $_SESSION['view']['cnt'])
{
  ...
  { 
    ...
    {    
      ...
      $db->query("SELECT * FROM  wmrush_serfing WHERE id = '".$_SESSION['view']['id']."' and money >= price LIMIT 1"); 
   
      if ($db->NumRows()) 
      {  
        $result = $db->FetchAssoc();
        
        $db->query("SELECT id FROM wmrush_serfing_view WHERE ident = '".$_SESSION['view']['id']."' and user_id = '".$_SESSION['user_id']."' and time_add + INTERVAL 24*60*60 SECOND > NOW() LIMIT 1");
        ...
        $db->query("UPDATE wmrush_users_b SET `money_b` = `money_b` + '".$pay_user."'	WHERE id = '".$_SESSION['user_id']."'");
          
        //записываем просмотр списываем бабло
        $db->query("UPDATE wmrush_serfing SET `view` = `view` + '1', `money` = `money` - '".$price."'	WHERE id = '".$id."'");
             
        
        //записываем что просмотрена ссылка
        $db->query("SELECT id, ident FROM wmrush_serfing_view WHERE user_id = '".$_SESSION['user_id']."' and ident = '".$id."' LIMIT 1");
 
        if ($db->NumRows())
        { 
          $result_view = $db->FetchAssoc();
         
          $db->query("UPDATE wmrush_serfing_view SET time_add = NOW() WHERE id = '".$result_view['id']."'");        
        }
        else
        {
          $db->query("INSERT INTO wmrush_serfing_view (`ident`, `time_add`, `user_id`) VALUES ('".$id."', NOW(), '".$_SESSION['user_id']."')");                                  
        }
