<div class="s-bk-lf">
	<div class="acc-title3">История пополнений</div>
</div>
...
if(isset($_GET["last_31"])){
	...
	$db->Query("SELECT * FROM ".$pref."_insert_money WHERE date_add > $dlim ORDER BY id DESC");
...
if(isset($_GET["list_day"])){
	...
	$db->Query("SELECT * FROM ".$pref."_insert_money ORDER BY id DESC");
...
$tdadd = time() - 5*60;
	if(isset($_POST["clean"])){
		...
		$db->Query("DELETE FROM ".$pref."_insert_money WHERE date_add < '$tdadd'");
...
<center><input type="submit" name="clean" value="Очистить" class="btn_8"/></center>
</form>
<?PHP
