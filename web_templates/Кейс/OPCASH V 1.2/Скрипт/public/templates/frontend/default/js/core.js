...
$(document).ready(function(){
    ...
    $('#join').click(function() {
      $.ajax({
        ...
        success: function(rdata){
          if(rdata.limit == 'true'){
            smoke.alert("Open 10 cases before entering free giveaway!");
          }
          if('true' == rdata.status){
              smoke.alert("Joined free giveaway!");
              location.reload();
          }else if(rdata.status == 'false' && rdata.joined == 'true') {
                smoke.alert("You already joined mate let others to join too!");
          }else if(rdata.status == 'false' && rdata.login == 'true') {
                smoke.alert("Login first!");
          }
    ...
    socket.on('last gifts', function (data_gifts) {
        ...
        $.each(data_gifts, function(i) {
            ...
            if (data_lgift['user'] != undefined && data_lgift['gift'] != undefined) {
                live_gifts_list += '<div class="b"><div class="winner-box"><div class="winner"><a href="/profile/'+data_lgift['user_id']+'"><img src="'+data_lgift['user_avatar']+'" alt="'+data_lgift['gift']+'"></a></div></div><img src="'+data_lgift['gift_img']+'" alt="150"></div>';
            }
