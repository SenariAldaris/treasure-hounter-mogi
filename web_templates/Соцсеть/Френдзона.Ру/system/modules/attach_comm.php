...
//* Выводим данные о комментариии *//
			...
			$row = $db->super_query("SELECT tb1.forphoto, auser_id, tb2.ouser_id FROM `".PREFIX."_attach_comm` tb1, `".PREFIX."_attach` tb2 WHERE tb1.id = '{$id}' AND tb1.forphoto = '{$purl}' AND tb2.photo = '{$purl}'");
			$tab_photos = false;
