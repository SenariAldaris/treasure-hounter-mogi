...
if(empty($caseName)){
    ...
    if($case->num_rows == 0){
        ...
        foreach($items as $key => $item){
            ...
            https://http://steamcommunity.com/market/search?appid=730&q='.$searchQuery.'
            $itemsBlock .= '<a class="item-incase '.$item->type.'" style="text-decoration: none;">
			<div class="picture">
...
echo $Functions->getIndex("case", [
    'from' => ['{case_title}', '{case_in}', '{rand_cases}', '{case_name}', '{case_picture}', '{total_case}', '{total_users}', '{online_people}', '{case_price}'],
    'to' => [$case->title, $itemsBlock, $caruselBlock, $case->name, $case->picture , $totalcase, $totaluser, sizeof(file($base)),$case->price]
]);
