...
var reg = {
			...
			if(isValidName(name)){
				if(lastname != 0){
					if(isValidName(lastname)){
						...
						$('#err').show("fast").html(lang_nosymbol);
					}
				} else {
					setErrorInputMsg('lastname');
					$('#err').show("fast").html(lang_empty);
				}
			} else {
		...
		if(email != 0 && isValidEmailAddress(email)){
			if(new_pass != 0 && new_pass.length >= 6){
				if(new_pass == new_pass2){
					...
					$('#err2').show("fast").html('Оба введенных пароля должны быть идентичны.');
				}
			} else {
				setErrorInputMsg('new_pass');
				$('#err2').show("fast").html('Длина пароля должна быть не менее 6 символов.');
			}
		} else {
			setErrorInputMsg('email');
			$('#err2').show("fast").html(lang_bad_email);
		}
	},
	send: function(sec_code){
		...
		$.post('/index.php?go=register', {
			...
			var exp = d.split('|');
			if(exp[0] == 'ok'){
				window.location = '/editmypage';
			} else if(exp[0] == 'err_mail'){
				$('#err2').show("fast").html('Пользователь с таким E-Mail адресом уже зарегистрирован.');
				Box.Close('sec_code');
			} else {
