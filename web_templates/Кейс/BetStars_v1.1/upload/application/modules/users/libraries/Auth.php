...
class Auth
{
     ...
     *
     * @param string  $username    The user's username credentials.
    ...
    public function login($username, $password, $remember = false)
    {
        ...
        $user = $this->ci->user_model->find_by(array('username' => $username));


        // Check whether the username, email, or password doesn't exist.
        if ($user == false) {
			Template::set_message(lang('us_bad_pass'),'error');
            return false;
        }
     ...
     */
    public function num_login_attempts($username = null)
    {
     ...
     *
     * @param string  $username   The login credentials (typically username).
     * @param integer $expires The expiration time (in seconds). Attempts older
     * than this value will be deleted.
     *
     * @return void
     */
    protected function clear_login_attempts($username, $expires = 86400)
    {
        $this->ci->db->where(array('ip_address' => $this->ip_address, 'username' => $username))
     ...
     *
     * @param string $username The login id used (typically username).
     *
     * @return void
     */
