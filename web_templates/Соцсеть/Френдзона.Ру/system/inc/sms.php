...
$sql_ = $db->super_query("SELECT tb1.user_id, from_u, abonent_cost, date, tb2.user_search_pref, balance_rub FROM `".PREFIX."_sms_log` tb1, `".PREFIX."_users` tb2 WHERE tb1.user_id = tb2.user_id {$sql_where} ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}", 1);
