...
var format_time = function(timestamp, size)
{
	...
	return (diff < 0)? 'in ' + seconds2time(-diff, size) : '<span class="ui-label-red">Expired</span>';
}
...
{	
	...
	if(size == "L")
	{
		...
		if(weeks == 1)	buffer += ' week ';
		if(days > 1)	buffer += days + ' days ';
		if(days == 1)	buffer += days +' day ';
		if(hours > 1)	buffer += hours + ' hours ';
		if(hours == 1)	buffer += hours + ' hour ';
		if(minutes > 1)	buffer += minutes + ' minutes ';
		if(minutes == 1)buffer += minutes + ' minute';
	}else{
