...
$rating = $this->bookmakers_model->get_rating($bookmaker_id);
$noReviews = $this->bookmakers_model->noReviews($bookmaker_id);?>
...
<div class="box">
	...
	if(isset($reviews) && is_array($reviews) && count($reviews))
	{
		...
		foreach ($reviews as $row)
		{
			...
			if (!empty($current_user))
			{
				...
				$user_votes = $this->bookmakers_model->user_votes($row->review_id,$user_id);
				if ($user_votes->num_rows() > 0)
				{
					$url = base_url('bookmakers');
					$vote_sum = $this->bookmakers_model->vote_sum($row->review_id);
	...
	if (!empty($current_user))
	{
		...
		$user_id = $current_user->id;  
		$query = $this->bookmakers_model->user_Reviews($user_id, $bookmaker_id);
