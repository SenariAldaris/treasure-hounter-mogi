...
$getfields = array(
	...
	'questions'=>array(
		...
		array('id','category','answerer','title','text','answer','author','tradeurl')
		),
);

$listsql = array(
	'answerers'=>"SELECT id AS ARRAY_KEY,CONCAT(trade_url) AS `last_name` FROM users_shop",
	'categories'=>"SELECT id AS ARRAY_KEY,title FROM ?_question_categories",
	'tradeurl'=>"SELECT id AS ARRAY_KEY,CONCAT(first_name,' ',last_name) AS `last_name` FROM users_shop",
);
