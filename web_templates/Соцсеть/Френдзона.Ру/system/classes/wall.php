...
class wall {
	...
	function select(){
		...
		foreach($this->query as $row_wall){
			...
			$tpl->set('{tell_num}', $row_wall['tell_num']);
...
//* Кнопка Показать полностью *//
			...
			if($textLength > 9 OR $strTXT > 600 AND stripos($row_wall['text'], '<!-->') === false)
...
//* Фото со стены юзера *//
						...
						if($attach_type[1] == 'attach' AND file_exists(ROOT_DIR."/uploads/attach/{$attauthor_user_id}/c_{$attach_type[2]}")){
							...
							if($cnt_attach == 1)
								...
								$attach_result .= "<img id=\"photo_wall_{$row_wall['id']}_{$cnt_attach}\" src=\"/uploads/attach/{$attauthor_user_id}/c_{$attach_type[2]}\" align=\"left\" onClick=\"groups.wall_photo_view('{$row_wall['id']}', '', '{$attach_type[1]}', '{$cnt_attach}')\" class=\"cursor_pointer photo_user_regular page_num{$row_wall['id']}\" height=\"{$rodImHeigh}\" />";
							...
							if($cnt_attach < 2)
								...
								$attach_result .= "<img id=\"photo_wall_{$row_wall['id']}_{$cnt_attach}\" src=\"/uploads/users/{$attauthor_user_id}/albums/{$attach_type[2]}/c_{$attach_type[1]}\" align=\"left\" onClick=\"groups.wall_photo_view('{$row_wall['id']}', '{$row_wall['tell_uid']}', '{$attach_type[1]}', '{$cnt_attach}')\" class=\"cursor_pointer photo_user_regular page_num{$row_wall['id']}\" />";
...
//* Видео *//
						...
						if($cnt_attach_video == 1 AND preg_match('/(photo|photo_u)/i', $row_wall['attach']) == false){
							...
							$row_video['video'] = strtr($row_video['video'], array('width="770"' => 'width="560"', 'height="420"' => 'height="370"'));
							
							$attach_result .= "<div class=\"cursor_pointer clear\" id=\"no_video_frame{$video_id}\" onClick=\"$('#'+this.id).hide();$('#video_frame{$video_id}').show();\">
							<div class=\"video_inline_icon\"></div><img src=\"/uploads/videos/{$attach_type[3]}/{$attach_type[1]}\" style=\"margin-top:3px\" width=\"560\" height=\"390\" /></div><div id=\"video_frame{$video_id}\" class=\"no_display\" style=\"padding-top:3px\">{$row_video['video']}</div><div class=\"video_inline_vititle\"></div><a href=\"/video{$attach_type[3]}_{$attach_type[2]}\" onClick=\"videos.show({$attach_type[2]}, this.href, location.href); return false\"><b>{$row_video['title']}</b></a>";
...
<div class="wall_tell_infow"><div class="wall_tell_ava"><a href="/{$tell_link}{$row_wall['tell_uid']}" onClick="Page.Go(this.href); return false"><img src="{$avaTell}" width="30" /></a></div><div class="wall_tell_name"><a href="/{$tell_link}{$row_wall['tell_uid']}" onClick="Page.Go(this.href); return false"><b>{$rowUserTell['user_search_pref']}</b></a></div><div class="wall_tell_date">{$dateTell}</div></div>{$row_wall['text']}
<div class="clear"></div>
</div>
HTML;
			}
...
//* Показ кнопки "Рассказать др" только если это записи владельца стр *//
...
//* Аватарка *//
			...
			if($user_info['user_photo']){
				...
				$tpl->set('{ava3}', '/templates/Old/images/no_ava_50.png');
...
//* Помещаем все комменты в id wall_fast_block_{id} это для JS *//
			...
			$tpl->result[$this->compile] .= '<div class="boxss" id="wall_fast_block_'.$row_wall['id'].'">';
...
//* Загружаем кнопку "Показать N записи" если их больше 3 *//
