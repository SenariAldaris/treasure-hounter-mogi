...
if(!CheckFriends($for_user_id)) AntiSpam('messages');
...
//* Проверка есть ли запрашиваемый юзер в друзьях у юзера который смотрит стр *//
					...
					if($user_info['user_group'] == 1) $xPrivasy = 1;
...
//* Обновляем кол-во новых сообщений у получателя *//
...
//* Отправка уведомления на E-mail *//
						...
						if($config['news_mail_8'] == 'yes' AND $user_id != $for_user_id){
							...
							if($rowUserEmail['user_email'] AND $EMAIL_block_data['n_im']){
...
//* Просмотр истории сообещений с юзером *//
		...
		case "history":
			...
			$sql_ = $db->super_query("SELECT tb1.id, text, date, pm_read, folder, history_user_id, tb2.user_name, short_link FROM `".PREFIX."_messages` tb1, `".PREFIX."_users` tb2 WHERE tb1.for_user_id = '{$user_id}' AND tb1.from_user_id = '{$for_user_id}' AND tb1.history_user_id = tb2.user_id ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}", 1);
...
//* Запрос в базу данных на вывод сообщений *//
				
				$query = "SELECT tb1.id, theme, text, for_user_id, from_user_id, date, pm_read, attach, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile, short_link FROM `".PREFIX."_messages` tb1, `".PREFIX."_users` tb2 WHERE tb1.for_user_id = '{$user_id}' AND tb1.folder = 'inbox' AND tb1.from_user_id = tb2.user_id {$search_sql} ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}";
				$sql_ = $db->super_query($query, 1);
				
//* Если есть ответ из базы данных, то считаем кол-вот ответа *//
