[record]<div class="wallrecord wall_upage"   id="wall_record_{rec-id}" style="padding-bottom:4px;margin-top:5px">
 ...
 <div class="wallauthoruu fl_l"><div style="float:left;width:45px"><div class="ava_minis [privacy-comment][if-comments]wall_ava_minisw[/if-comments][/privacy-comment]" id="ava_rec_{rec-id}"><a href="/u{user-id}" onClick="Page.Go(this.href); return false"><img src="{ava}" alt="" title="" /></a><div class="kolsw"></div></div></div><a href="/u{user-id}" onClick="Page.Go(this.href); return false"> <div class="wallauthoruu_name">{name} <span class="color777">{type}</span></div></a><a href="/wall{author-id}_{rec-id}" onClick="Page.Go(this.href); return false" class="online"><div class="datesw">{date} </div></a><div id="kolsw_box" class="kolsw_box">
...
[all-comm]<div class="cursor_pointer" onClick="wall.all_comments('{rec-id}', '{author-id}'); return false" id="wall_all_but_link_{rec-id}"><div class="public_wall_all_comm_gred" id="wall_all_comm_but_{rec-id}">Показать {gram-record-all-comm}</div></div>[/all-comm]
[comment]<div class="wall_fast_blocks" id="wall_fast_comment_{comm-id}" onMouseOver="ge('fast_del_{comm-id}').style.display = 'block'" onMouseOut="ge('fast_del_{comm-id}').style.display = 'none'">
