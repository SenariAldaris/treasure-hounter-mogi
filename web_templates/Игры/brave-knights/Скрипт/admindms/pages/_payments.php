<div class="s-bk-lf">
	<div class="acc-title3">Список выплат на Payeer</div>
</div>
<div class="silver-bk"><div class="clr"></div>	
<center><a href="index.php?menu=payments">Выплачено</a> || <a href="?menu=payments&balance">Баланс на Payeer</a> || 
...
if(isset($_GET["last_31"])){
	...
	$db->Query("SELECT * FROM ".$pref."_payment WHERE date_add > $dlim ORDER BY id DESC");
...
if(isset($_GET["list_day"])){
	...
	$db->Query("SELECT * FROM ".$pref."_payment ORDER BY id DESC");
...
$db->Query("SELECT * FROM ".$pref."_payment ORDER BY id DESC LIMIT {$lim}, 100");
...
<?PHP
	...
	?>
	<tr class="htt">
	<td align="center"><a href="index.php?menu=users&edit=<?=$data["user_id"]; ?>" class="stn"><font color = '#914A1F'><?=$data["user"]; ?></font></a></td>
...
$db->Query("SELECT COUNT(*) FROM ".$pref."_payment");
